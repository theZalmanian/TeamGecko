...
function swapTheme() {
    ...
    // if the theme is set to dark mode
    if(themeIsDarkMode()) {
        // remove the theme attribute, returning the page to the default: light mode
    ...
    else if(!themeIsDarkMode()) {
        // set the page to dark mode by adding the theme attribute set to dark
...
/**
 * Checks if the current page's theme is set to dark mode, and returns true/false accordingly
 * @returns {boolean} True if page is set to dark mode; otherwise False
 */
function themeIsDarkMode() {
    // grab the HTML root of the current page
    const pageHTMLRoot = document.documentElement;

    // If it has the theme declaration attribute, and it is set to dark-mode, the theme is dark mode
    return pageHTMLRoot.hasAttribute(THEME_ATTRIBUTE) && pageHTMLRoot.getAttribute(THEME_ATTRIBUTE) === "dark";
}

/**
 * Sets the icon of the "Theme Switcher" button to the given SVG path
 * @param {string} svgPath The path of the SVG icon being displayed in the "Theme Switcher" button
 */
...
/**
 * If page is being accessed on mobile and the navbar is expanded, collapses it
 */
